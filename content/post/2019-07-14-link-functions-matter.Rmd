...
There's a bit of a conversation on Twitter whether to use linear models or logistic models for estimating effect sizes. I wondered if linear models can work for prediction problems, since I thought there may be cases where the latent probability is linearly related to the features. 
...
Preprocessing steps are taken from Megan Risdal's R kernel on Kaggle, [Exploring the Titanic Dataset](https://www.kaggle.com/mrisdal/exploring-survival-on-the-titanic), with the one change being that I use bagging imputation instead of MICE imputation for the Age column. 
